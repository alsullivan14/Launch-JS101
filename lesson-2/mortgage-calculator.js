...
// Accept loan amount as integer or floats, rounding to two decimal places if necessary
// Accept loan duration as number of months or years. Convert to months if not already
...
let monthsOrYears = rl.prompt();
let durationInMonths;

// Require months or years to be entered and convert result to months
if (monthsOrYears === "Y".toLowerCase()) {
  durationInMonths = loanDuration * 12;
} else if (monthsOrYears === "M".toLowerCase()) {
  durationInMonths = loanDuration;
} else {
  console.log("Please enter 'm' or 'y': ");
  monthsOrYears = rl.prompt();
}
...
let monthlyPayment = loanAmount * (monthlyInterestRate / (1 - Math.pow((1 + monthlyInterestRate), (-durationInMonths))));
